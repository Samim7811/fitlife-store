...
type Order={id:string;tracking_number?:string;customer_name?:string;customer_phone?:string;address?:string;city?:string;state?:string;pincode?:string;landmark?:string;product_name?:string;quantity?:number;total_amount?:number;amount?:number;status?:string;payment_method?:string;created_at?:string};
...
export default function OrdersPage(){
 ...
 return <main className="page">
  ...
  <div className="list">{list.map(o=><div className="card" key={o.id}>
   <div className="row"><b>Order #{o.tracking_number || o.id.slice(0,8)}</b><span className={`status ${o.status||"pending"}`}>{o.status||"pending"}</span></div>
   <h3>{o.customer_name||"Customer"}</h3><p>📞 {o.customer_phone||"N/A"}</p>
   <div className="address">
    <b>📍 Delivery Address</b>
    <p>{o.address||"Address not available"}</p>
    <p>{o.city||""}{o.city && o.state ? ", " : ""}{o.state||""} {o.pincode||""}</p>
...
.top{display:flex;justify-content:space-between;align-items:center;gap:12px;margin-bottom:20px}.top h1{margin:0;font-size:28px}.top p{color:#6b7280;margin:6px 0}.top b{background:#111827;color:white;padding:9px 12px;border-radius:10px;font-size:13px}
   ...
   .list{display:grid;gap:12px}.card{border:1px solid #e5e7eb;border-radius:16px;padding:16px;box-shadow:0 3px 12px #0000000b}.row,.bottom{display:flex;justify-content:space-between;align-items:center}.card h3{margin:13px 0 6px}.card p{margin:6px 0;color:#6b7280;font-size:14px}.address{margin:12px 0;padding:12px;border-radius:10px;background:#f8fafc;border:1px solid #e5e7eb}.address b{font-size:14px}.address p{margin:5px 0;color:#374151}.bottom{margin-top:14px;border-top:1px solid #eee;padding-top:12px}.bottom b{font-size:18px}.bottom small{color:#888}
